...
def pinv_by_tikhonov(frf_matrix,
    ...
    if regularization_parameter.size > 1 and regularization_parameter.size != frf_matrix.shape[0]:
        raise ValueError('The regularization parameter is a vector but the size does not match the number of frequency lines and cannot be broadcasted to the FRFs') 
    ...
    if regularization_weighting_matrix is not None and regularization_parameter.size > 1 and regularization_weighting_matrix.ndim == 2:
        warnings.warn('The regularization weighting matrix is 2D while the regularization parameter is a vector, the regularization weighting matrix is being expanded to 3D. Consider adding a third dimension to the regularization weighting matrix as appropriate')
    ...
    if regularization_weighting_matrix is not None and regularization_weighting_matrix.ndim == 3 and regularization_parameter.size > 1 and regularization_parameter.ndim != 3:
        regularization_parameter = regularization_parameter[..., np.newaxis]
...
def pinv_by_truncation(frf_matrix, 
    ...
    if num_retained_values.size > 1 and num_retained_values.size != frf_matrix.shape[0]:
        raise ValueError('The number of retained values parameter is a vector but the size does not match the number of frequency lines and cannot be broadcasted to the FRFs') 
    ...
    if num_retained_values.size > 1 and any(num_retained_values > s.shape[1]) or num_retained_values.size == 1 and num_retained_values > s.shape[1]:
        warnings.warn('The requested number of retained singular values is greater than the actual number of singular values (for at least one frequency line). All singular values are being retained.')
