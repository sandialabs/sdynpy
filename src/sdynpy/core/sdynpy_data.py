    ...
    def __init__(self, *data_to_plot, **labeled_data_to_plot):
        ...
        **labeled_data_to_plot : NDDataArray
        ...
        if self.number_of_datasets == 2:
            ...
            tab_b = matplotlib.colormaps['tab20b'].resampled(15)(np.linspace(0, 1, 15))
            tab_c = matplotlib.colormaps['tab20c'].resampled(15)(np.linspace(0, 1, 15))
            self.cm = ListedColormap(np.concatenate((tab_c, tab_b), axis=0))
            self.cm_mod = 30
        elif self.number_of_datasets == 4:
            # Combine tab20b and tab20c
            tab_b = matplotlib.colormaps['tab20b'].resampled(20)(np.linspace(0, 1, 20))
            tab_c = matplotlib.colormaps['tab20c'].resampled(20)(np.linspace(0, 1, 20))
            self.cm = ListedColormap(np.concatenate((tab_c, tab_b), axis=0))
            self.cm_mod = 40
        else:
            self.cm = cm.tab10
